...
def cat_items(items):
    ...
    # 将多个 batch 的 items 堆叠在一起
    items = default_collate(items)
    # stack -> cat：将堆叠的 items 重新拼接
...
def check_last_batch(it):
    '''
    输入：迭代器
    返回：(item, is_last_batch)
    标记最后一个 batch，用于判断是否需要执行某些操作（如保存 checkpoint）
    '''
    last = None
    for x in it:
        if last is not None:
...
def train_one_epoch(model, data, epoch, optimizer, scaler, scheduler, scheduler_l0, args, tb_writer=None, start_iter=0, zs=None):
    ...
    if start_iter == 0:
        # 通过 sampler 或 shared_epoch 以进程安全的方式设置 epoch
    ...
    if args.distributed and sampler is not None and start_iter == 0:
        # [不要删除！] 即使 sampler 不是 DistributedSampler 也会调用 set_epoch
    ...
    for (i, batch), is_last_batch in check_last_batch(enumerate(dataloader, start=start_iter)):
        ...
        '''
        潜在 bug：
            有三个分支：image、text、logit
            每个优化器都有自己的 `found_inf_per_device`。
            三个 `found_inf_per_device` 应该同步，否则某个分支会使用错误的梯度进行更新？
        '''
        # 检查损失
        for opt in used_optimizer:
            scaler.unscale_(opt)

        # 同步 found_inf_per_device
        found_inf = sum(
        ...
        # 注意：按照原始论文，将 logit_scale 限制在 4.6052 = ln(100) 以内
        with torch.no_grad():
            if args.logit_scale is not None:
        ...
        if is_master(args) and (i % 10 == 0 or is_last_batch):
            ...
            # 注意：损失是粗略采样的，仅在 master 节点和每次日志更新时记录
            ...
            # 保存训练损失等指标。使用非平均计量器的值，因为日志记录器有自己的平滑逻辑
            log_data = {
                "loss": loss_m.val,
