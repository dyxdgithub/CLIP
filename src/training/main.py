...
def main():
    ...
    if torch.cuda.is_available():
        # 在 Ampere GPU 上启用 tf32，比 float16 慢约 8%，但几乎和 float32 一样准确
        # 这是 PyTorch 1.12 之前的默认设置
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.backends.cudnn.benchmark = True
        torch.backends.cudnn.deterministic = False

    # 清理模型名称以适应文件系统/URI，避免名称中包含 /
    ...
    model, preprocess_train, preprocess_val = create_model_and_transforms(
        args.model,
        args.pretrained,
        # 如果 args.precision 是 fp16，模型将被转换为 FP16
    ...
    if args.lock_image:
        # 锁定图像编码器（参考 LiT 论文 - https://arxiv.org/abs/2111.07991）
    ...
    if args.resume is not None:
        # 此部分仅支持恢复不含 mask 的 CLIP 模型。[TODO]：支持恢复带 mask 的 CLIP 模型。
